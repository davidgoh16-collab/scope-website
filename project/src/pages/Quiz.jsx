...
const QUIZ_QUESTIONS = [
  {
    id: 'role',
    label: 'About you',
    title: "Who's taking the quiz?",
    sub: "We'll tailor the questions to match.",
    type: 'single',
    options: [
      { id: 'student', label: "I'm the student", sub: 'Answering for myself', icon: '🙋' },
      { id: 'guardian', label: 'Parent or guardian', sub: 'Helping someone else', icon: '👨‍👩‍👧' },
    ],
  },
  {
    id: 'year',
    ...
    type: 'single',
    guardian: { title: "Which year is your child in?" },
    options: [
      { id: 'y6', label: 'Year 6', sub: 'Looking at secondary schools', icon: '📚' },
      { id: 'y7', label: 'Year 7', sub: 'In-year secondary move', icon: '🏫' },
      { id: 'y8', label: 'Year 8', sub: 'In-year secondary move', icon: '🏫' },
      { id: 'y9', label: 'Year 9', sub: 'In-year secondary move', icon: '🏫' },
      { id: 'y10', label: 'Year 10', sub: 'In-year secondary move', icon: '🏫' },
      { id: 'y11', label: 'Year 11', sub: 'Looking at sixth forms & colleges', icon: '🎓' },
      { id: 'y12', label: 'Year 12', sub: 'In-year sixth form move', icon: '🎓' },
      { id: 'y13', label: 'Year 13', sub: 'In-year sixth form move', icon: '🎓' },
    ],
  },
  {
    id: 'postcode',
  ...
  },
  {
    ...
    type: 'multi',
    guardian: { title: "Which subjects does your child actually look forward to?" },
    options: [
  ...
  },
  {
    ...
    type: 'single',
    guardian: { title: "What kind of lessons make your child actually want to show up?" },
    options: [
  ...
  },
  {
    ...
    type: 'single',
    guardian: { sub: "Some schools are a perfect fit but might be further. How far is too far?" },
    options: [
  ...
  },
  {
    ...
    sensitive: true,
    guardian: {
      title: 'Is there anything we should know about how your child learns best?',
      options: [
        { id: 'ehcp', label: 'They have an EHCP', icon: '📋' },
...
const QuizPage = ({ navigate }) => {
  ...
  const isGuardian = answers.role === 'guardian';
  const resolveQ = (question) =>
    isGuardian && question.guardian
      ? { ...question, ...question.guardian, options: question.guardian.options || question.options }
      : question;
  const q = resolveQ(QUIZ_QUESTIONS[step - 1]);
  ...
  <Section pad="96px 0" style={{ background: 'linear-gradient(180deg, var(--blue-50) 0%, #fff 100%)' }}>
    <div className="container-narrow" style={{ textAlign: 'center' }}>
      ...
      <p className="lead" style={{ margin: '0 auto 40px', fontSize: 19 }}>
        Just a few plain-English questions. You can pause at any point, skip optional questions, and edit your answers later. Nothing leaves your device unless you choose to save your results.
      </p>
...
const QuizResults = ({ answers, navigate, onRetake }) => {
  ...
  const yearOptions = QUIZ_QUESTIONS.find(question => question.id === 'year')?.options || [];
  const yearLabel = yearOptions.find(opt => opt.id === answers.year)?.label || 'Browsing';
        ...
        <div className="container-narrow">
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <span className="chip chip-purple">{yearLabel}</span>
